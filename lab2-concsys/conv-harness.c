// Team: Mykhailo Bitiutskyy, Anand Sainbileg, Andrii Yupyk 
...
void student_conv(float *** image, int16_t **** kernels, float *** output,
  ...
  int m, w, h, x, y, c;
  float **** _kernels = new_empty_4d_matrix_float(nkernels, kernel_order, kernel_order, nchannels);

  // Convert kernels to float matrix with [nkernels][kernel_order][kernel_order][nchannels] structure
...
int main(int argc, char ** argv) {
  float ***image;
  int16_t ****kernels;
  float ***davids_output, ***students_output;
  int width, height, kernel_order, nchannels, nkernels;
  struct timeval start_time, stop_time;
  long long davids_time, students_time;
  ...
  students_output = new_empty_3d_matrix_float(nkernels, width, height);
  davids_output = new_empty_3d_matrix_float(nkernels, width, height);

  // Run David's code
  gettimeofday(&start_time, NULL);
  multichannel_conv(image, kernels, davids_output, width, height, nchannels, nkernels, kernel_order);
  gettimeofday(&stop_time, NULL);
  davids_time = (stop_time.tv_sec - start_time.tv_sec) * 1000000L + (stop_time.tv_usec - start_time.tv_usec);

  // Run student's code
  gettimeofday(&start_time, NULL);
  student_conv(image, kernels, students_output, width, height, nchannels, nkernels, kernel_order);
  gettimeofday(&stop_time, NULL);
  students_time = (stop_time.tv_sec - start_time.tv_sec) * 1000000L + (stop_time.tv_usec - start_time.tv_usec);

  // Print results
  DEBUGGING(write_out(students_output, nkernels, width, height));
  check_result(students_output, davids_output, nkernels, width, height);
  printf("David's conv time: %lld microseconds\n", davids_time);
  printf("Student's conv time: %lld microseconds\n", students_time);
  printf("Gain over David's conv time: %.2f times\n", (double) davids_time / students_time);
